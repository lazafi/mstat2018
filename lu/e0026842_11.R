...
# ->
cca <- function(X,Y,robust) {
  mcd <- covMcd(cbind(X,Y), cor = TRUE)
  
  S <- mcd$cor
  
  S_XX <- S[colnames(X), colnames(X)] 
  S_XY <- S[colnames(X), colnames(Y)]
  S_YY <- S[colnames(Y), colnames(Y)]
  S_YX <- S[colnames(Y), colnames(X)]
  
  CX <- solve(S_XX) %*% S_XY %*% solve(S_YY) %*% S_YX
  CY <- solve(S_YY) %*% S_YX %*% solve(S_XX) %*% S_XX
  
  correlation <- sqrt(eigen(CX)$values)
  
  X_coef <- eigen(CX)$vectors
  Y_coef <- eigen(CY)$vectors

  res <- list("cor" = correlation, "A" = X_coef, "B" = Y_coef)
  res
  
}

cca_rob <- cca(fitness.data.raw[,i], fitness.data.raw[,j])
plot(fitness.data.raw[,i] %*% cca_rob$A, fitness.data.raw[,j] %*% cca_rob$B)
fitness.data.raw[,i]
cca_rob$A[,1]
#fitness.data.raw[,i] %*% cca_rob$A[,1]